...
class LocationController
{
    ...
    public function store(Request $request, Response $response): Response
    {
        try {
            ...
            $coverImageUrl = $this->extractLocationImageUrl($request, $data);
            $location = $this->locationService->create($data);

            if ($coverImageUrl) {
                $this->locationService->addImage((int)$location->id, [
                    'image_url' => $coverImageUrl,
                    'alt_text' => $location->name,
                    'is_main' => 1,
                ])->setAsMain();
                $location = $this->locationService->getById((int)$location->id);
            }
    ...
    private function extractLocationImageUrl(Request $request, array $data): ?string
    {
        ...
        if (isset($uploadedFiles['image']) && $uploadedFiles['image'] instanceof UploadedFileInterface) {
            ...
            if ($file->getError() === UPLOAD_ERR_OK) {
                return $this->uploadService->saveLocationImage($file);
            }
        }
